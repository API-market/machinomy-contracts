...
contract CPU is MintableToken, BurnableToken, Pausable {
  ...
  event BuyCPU(address  _from, address _to, uint _value);
  event SellCPU(address _from, uint _amount);
  event DeliverCPU(address _to, uint _amount);
  event PickupCPU(address _from, uint _value);
  ...
  function sellCPU(uint _amount) public returns (bool) {
    require(msg.sender != address(0));
    require(_amount > 0);
  ...
  function deliverCPU(address _to, uint _amount, uint _value) onlyOwner public returns (bool) {
    require(_to != address(0));
  ...
  function pickupCPU(address _from, uint _amount) onlyOwner public payable returns (bool) {
